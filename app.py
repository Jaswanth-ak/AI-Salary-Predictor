...
# HEADER
# ──────────────────────────────────────────────
st.title("💼 AI Salary Prediction and Analytics System")
...
with st.sidebar:
    ...
    selected_residence_name = st.selectbox(
        "Employee Residence",
        residence_display
    )

    employee_residence = next(
        code for code, name in COUNTRY_NAMES.items()
        if name == selected_residence_name
    )
    ...
    company_location = next(
        code for code, name in COUNTRY_NAMES.items()
        if name == selected_location_name
    )

    company_size = st.selectbox("Company Size", ["S", "M", "L"])

    remote_ratio = st.select_slider("Remote Ratio", [0, 50, 100])
